...
function(data,ndim=2,verbose=FALSE,itmax=125,eps=1e-12){
  ...
  dis <- .jcall(hjw, "[[D","slidevectormodel",.jarray(data,dispatch=TRUE),as.integer(ndim),verbose,as.integer(itmax),as.double(eps))
  ...
  result<-list(ndim=ndim,stress=stress,confi=mat,slvec=Z,resid = resid, niter=niter,
               nobs=nobs,pred=pred,model="Slide-vector model")
